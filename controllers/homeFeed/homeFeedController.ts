...
export const postCurrentLoggedInUserInfo = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    if (!user) {
      res.status(404).json({
        message:
          '현재 홈 화면에서 유효한 user 정보를 DB에서 찾는 것에 실패했습니다.',
        status: 404,
      });
    }
...
export const getFeedsQueue = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    if (userId) {
      ...
      const productIds = userMemberships.map(
    ...
    const feed1Arr = _.shuffle(
      feeds.filter((feed) => feed.visibility_level === 1).map((feed) => feed.id)
    );
    const feed2Arr = _.shuffle(
      feeds.filter((feed) => feed.visibility_level === 2).map((feed) => feed.id)
    );
    const feed3Arr = _.shuffle(
      feeds.filter((feed) => feed.visibility_level === 3).map((feed) => feed.id)
    );
...
export const postFeedDatatoInfluencerData = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
...
export const patchUserFollow = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    if (!user) {
      res.status(404).json({
        message:
          '현재 로그인된 유저 정보와 일치하는 유효한 user 정보가 DB에 없습니다.',
        status: 404,
      });
      return;
    }
...
export const patchFeedLikes = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
...
export const postInfluencerUsernameInfo = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    if (!user) {
      res.status(404).json({
        message:
          '피드의 아이디를 통해 인플루언서를 찾았으나 유효한 유저가 아닙니다.',
        status: 404,
      });
    }
    ...
    console.error(
      '피드에서 인플루언서 id를 참고하여 user 테이블의 username을 가져오는 과정에서 문제가 생겼습니다: ',
      error
    );
    res.status(500).json({
      message:
...
export const postUserIdByInfluencerId = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
...
export const postUsernameByUserId = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
...
export const getMembershipProduct = async (
  ...
  try {
    ...
    console.error('getMembershipProduct ', error);
    res.status(500).json({
...
export const getUserMembershipLevel = async (
  ...
  try {
    ...
    console.error('getUserMembershipLevel ', error);
    res.status(500).json({
...
export const getMembershipProductsFromInfluencerId = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    console.error('getMembershipProductsFromInfluencerId ', error);
    res.status(500).json({
      message: 'getUserMembershipLevel',
      status: 500,
      error: error,
...
export const getMembershipFromUserId = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    console.error('getMembershipFromUserId ', error);
    res.status(500).json({
